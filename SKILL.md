...
| `C02` | `weekly_all_in_one` | `sequential` | 一键周报、完整周报、全量周报 | R09 -> R04 -> R03 -> R01 | 先采集数据，再抓进度快照，再扫风险，最后基于前三步的输出生成周报。上游输出自动注入下游上下文。 **容错逻辑**：若 milestones 为空导致 R04 触发上下文补全，先执行补全再继续后续路由，不中断整条流水线 |
...
| 模块文件 | 角色 | 一句话说明 |
|---------|------|-----------|
| `modules/pm-commander-extended.md` | PM Extended | PM 扩展模块 — 多项目管理、干系人映射、升级处理（自动加载为 PM 角色的补充能力） |
...
| 文件 | 说明 |
|------|------|
| `modules/manifest.yaml` | 模块注册表 — 定义所有模块的输入输出契约、路由映射和元数据 |
